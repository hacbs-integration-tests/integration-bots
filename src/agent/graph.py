...
5. If there was a current demo file in present (from step 1), move it to the old folder using move_file: use that file id and new_parent_id = OLD_FOLDER_ID below. If there was no file in present, skip this step.
6. Reply with a short summary: e.g. "Created Integration Team Sprint {{N+1}} Demo in present, updated slide to {{N+1}}, and moved the previous demo to old."

Folder IDs (use these in copy_file and move_file):
- PRESENT_FOLDER_ID: {present_folder_id}
- OLD_FOLDER_ID: {old_folder_id}

Use the tool results to extract file ids and the latest sprint number. For copy_file you need: file_id (template id), new_parent_id (PRESENT_FOLDER_ID above), new_name ("Integration Team Sprint X Demo" where X is N+1). After copy_file, use the returned new file id in update_first_slide_sprint_number with sprint_number = N+1. For move_file you need: file_id (previous demo from get_latest_sprint_number), new_parent_id (OLD_FOLDER_ID above).
...
def create_graph():
    ...
    tools = create_drive_tools(service, slides_service=slides_service)
